...
1.为了加了车的列表，引入了Vehicle类的动态数组，然而因为Vehicle内部已经有了一个维修记录的动态数组，因此编写合约时总是报错，无法通过编译。 最后查看文档，发现里面的范例对于有动态数组的结构体使用的是mapping，于是将Vehicle的动态数组改为mapping。

2.将Vehecle的动态数组改为mapping后，因为mapping和动态数组相比无length属性，因此需要额外维护一个记录所有车数量的全局变量。

3.在remix中部署编写的合约时，发现另一半的函数都无法部署，尝试了直接复制前面编译通过的函数到后边也不行，最后发现是remix中的gas限制的问题，函数过多导致部署需要的gas多，超出了limit。
